...
local protector_radius = tonumber(minetest.settings:get("protector_radius")) or 16
...
local protector_spawn = tonumber(minetest.settings:get("protector_spawn")
	or minetest.settings:get("protector_pvp_spawn")) or 16
local protector_show = tonumber(minetest.settings:get("protector_show_interval")) or 10
...
local protector_show_default_color = minetest.settings:get("protector_show_default_color") or "original"

-- A flag for if we want to "block" some stuff from being logged.
local protector_debug = false
...
-- Provide colored fields per protector
local get_color = function(meta)
    local color = meta:get_string("color")
    -- Ensure we have a valid color
    --[[if color ~= "white" or color ~= "black" or color ~= "blue" or
        color ~= "red" or color ~= "green" or color ~= "magenta" or
        color ~= "yellow" or color ~= "original" or color ~= "cyan" then
        -- Not a valid color, let's set it to a default
        color = protector_show_default_color
    end]]
    return color
end

-- Provide colored fields per protector
local set_color = function(meta, color)
    -- Ensure we have a valid color
    --[[if color ~= "white" or color ~= "black" or color ~= "blue" or
        color ~= "red" or color ~= "green" or color ~= "magenta" or
        color ~= "yellow" or color ~= "original" or color ~= "cyan" then
...
local protector_formspec = function(meta)
    ...
    -- I need to make a seperate formspec for selecting a different color
    local color = get_color(meta)
...
minetest.register_node("protector:protect", {
	...
	after_place_node = function(pos, placer)
		...
		meta:set_string("color", protector_show_default_color)
	...
	on_punch = function(pos, node, puncher)
        ...
        local meta = minetest.get_meta(pos)
		minetest.add_entity(pos, "protector:display_"..meta:get_string("color"))
	end,
	...
	if minetest.registered_items["default:stone"] and minetest.registered_items["default:wood"] then
		...
		minetest.register_craft({
			...
			recipe = {
				...
				{"mcl_core:stone", "mcl_core:wood", "mcl_core:stone"},
				{"mcl_core:stone", "mcl_core:stone", "mcl_core:stone"},
			}
		})
	end
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    -- Change color
	if fields.color_change then
        local current = meta:get_string("color")
        local colors = {
            "original", "black", "white", "blue",
        ...
        for i, c in pairs(colors) do
            if c == current then
                next_i = i + 1
                if next_i > 10 then
                    next_i = 2 -- Skip original (Should use magenta instead, if they want purple)
                end
                set_color(meta, colors[next_i])
                if protector_debug then
                    minetest.log("action", "[protector] Changed from "..current.." to "..meta:get_string("color").." at ("..pos.x..", "..pos.y..", "..pos.z..")")
                end
                break -- Stop looping (Change once not cascade)
            end
        end
    end
...
local make_displays = function()
    ...
    for _, c in pairs(colors) do
        if protector_debug then
            minetest.log("action", "[protector] Adding protector:display_"..c.." and protector:display_node_"..c.." with protector_display_"..c..".png")
        end
        -- display entity shown when protector node is punched
        minetest.register_entity("protector:display_"..c, {
	        physical = false,
	        collisionbox = {0, 0, 0, 0, 0, 0},
	        visual = "wielditem",
	        -- wielditem seems to be scaled to 1.5 times original node size
	        visual_size = {x = 0.67, y = 0.67},
	        textures = {"protector:display_node_"..c},
	        timer = 0,
	        glow = 10,
        ...
        minetest.register_node("protector:display_node_"..c, {
	        tiles = {"protector_display_"..c..".png"},
